...
{ // nekos_best::fetch
        if (argc < 3)
                {
                        printf (
                            "Usage: <search_query> [1|2] [category] [amount]");
                        return 1;
                }
        ...
        const auto &res = nekos_best::search (
            argv[1], (nekos_best::image_format)atoi (argv[2]),
            argc > 3 ? argv[3] : "", argc > 4 ? atoi (argv[4]) : 1);
        ...
        for (int i = 0; i < 5; i++)
                {
                        // very bad practice, you should create a new function
                        // instead of nesting everything here! LOOK HOW UGLY OMG
                        std::thread t ([&t_count, i] () {
                                for (int j = 0; j < 5; j++)
                                        {
                                                printf ("REQUEST %d %d\n",
                                                        i + 1, j + 1);
                                                // lock mutex to prevent data
                                                // races, but this is a rate
                                                // limit test so we don't need
                                                // it
                                                // std::lock_guard<std::mutex>
                                                // lk(nekos_best::ns_mutex);
                                                nekos_best::search (
                                                    "neko",
                                                    (nekos_best::
                                                         image_format)1,
                                                    "neko", 50);
                                                // const auto headers =
                                                // nekos_best::get_last_request_response().headers;
                                                // for (const auto data :
                                                // headers) { printf("'%s':
                                                // '%s'\n", data.first.c_str(),
                                                // data.second.c_str());
                                                // }
                                                const bool r = nekos_best::
                                                    is_rate_limited ("neko");
                                                printf (
                                                    "Is rate limited: %d\n",
                                                    r);
                                                if (r)
